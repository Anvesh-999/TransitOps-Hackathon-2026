    ...
    <div className="w-full max-w-md animate-fade-in relative px-4">
      ...
      <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl shadow-2xl p-8 relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-350 tracking-wider uppercase mb-1.5">Email Address</label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full px-4 py-3 bg-slate-950/50 border border-slate-800 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm text-white placeholder-slate-600"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-350 tracking-wider uppercase mb-1.5">Password</label>
            <div className="relative">
              <input
                type={showPassword ? 'text' : 'password'}
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                className="w-full px-4 py-3 bg-slate-950/50 border border-slate-800 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm text-white placeholder-slate-600 pr-10"
                placeholder="••••••••"
                required
                minLength={8}
              />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-blue-600 to-indigo-650 hover:from-blue-500 hover:to-indigo-600 text-white py-3 rounded-xl font-semibold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-sm shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 hover:scale-[1.01] active:scale-[0.99]"
          >
        ...
        <div className="mt-7 pt-6 border-t border-slate-800">
          <p className="text-[10px] font-bold text-slate-500 tracking-wider uppercase text-center mb-3.5">Quick Demo Login</p>
          <div className="grid grid-cols-2 gap-2.5 text-xs">
            {[
              ...
              <button
                ...
                className="text-left px-3.5 py-2.5 bg-slate-950/40 border border-slate-800 rounded-xl hover:bg-slate-900/50 hover:border-slate-700 transition-all duration-200"
              >
                <p className="font-semibold text-slate-200 mb-0.5">{cred.role}</p>
                <p className="text-slate-550 text-[10px] truncate">{cred.email}</p>
              </button>
            ))}
          </div>
        </div>
